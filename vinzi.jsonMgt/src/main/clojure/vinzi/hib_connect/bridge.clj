...
(defn props-createSessionFactory "Create a sessionFactory on the properties mentioned in the dbParam record." [{:keys [db resources]}]
  (let [lpf "(props-createSessionFactory): "]
    (letfn [(set-property [csf kv]
			;; set property given by the key-value pair to 'csf'
			(let [k (key kv)
			      v (val kv)
			      p (get csfPropMap k)]
			  (debug lpf " for key '" k "' set property '" p
				   "' to value: " v)
			  (if p
			    (let [ret (.setProperty csf p v)]
			      (debug lpf "  returning: " ret)
			      ret)
			    (do
			      (debug lpf "WARNING: Property: " k
				       "with value: " v
				       "is not recognized")
			      (debug lpf "   ... option ignored")
			      csf))))
            (add-resource [csf res]
                          ;; add a resource to the config 'csf'
                          (let [ret (.addResource csf res)]
                            (debug lpf "resource " res " returned: "ret) 
                            ret))]
           (debug lpf "Setting up a session factory with"
                  "\n\tParams: " db
                  "\n\tResources: " resources)
           (try
...
(defn xml-createSessionFactory "Create a sessionFactory based on the hibernate.settings.xml files at the class-path." []
  (let [lpf "(xml-createSessionFactory): "]
    (debug lpf "Setting up a hibernate session-factory from xml")
    (try
...
(defn force-open-hib
  ...
  []
  (let [lpf "(force-open-hib): "]
    (debug lpf "Force-open-hib:")
    (debug lpf "Currently using sessionFactory" sessionFactory)
    ;;(when sessionFactory
    ;;(debug lpf "  status " (.isClosed sessionFactory)))
    (when (or (nil? sessionFactory)
              (.isClosed sessionFactory))
      (def sessionFactory (createSessionFactory))
      (debug lpf "generated sessionFactory: " sessionFactory))))
  ...
  (let [lpf "(store-hib*): "]
    (letfn [(store-single [cRec]
                          ...
                          (let [jRec (trans-to-java cRec)]
                            (trace lpf  "saving the record with contents: " jRec)
                            (let [res (.save session jRec)
                                  newId (.getId jRec)]
                              (if (not= newId (:id cRec))
                                (do
                                  (trace lpf " id changed to: " newId)
                                  (assoc cRec :id newId))
                                cRec))))]
...
(let [lpf "(with-query-results*): "]
(when-not (vector? sql-params)
  (debug lpf "expected a vector for sql-params"))
(when (not= (count sql-params) 1)
  (debug lpf "The current implementation will only handle the first sql-param"
         "\n\tDiscarding: " (rest sql-params)))
;;
(trace lpf "about to run query: " (first sql-params)
       "\n\tNext applying local function " func "  containing body")
;;
...
(defn delete-hib*
  ...
  (let [lpf "(delete-hib): "]
    (letfn [(delete-single [cRec]
                           ;; translate the record to a java-object and store it
                           (let [jRec (trans-to-java cRec)]
                             (trace lpf  "Deleting the record with contents: " jRec)
                             (.delete session jRec)
                             1))]
           (let [res (if (= (type toDel) (type ""))
                       (let [res (.delete session toDel)]
                         (trace lpf "Deleting records for select-string: " toDel)
                         res)
                       (if (or (seq? toDel) (vector? toDel))
                         (count (doall (map delete-single toDel)))
                         (delete-single toDel)))]
             (trace lpf "Deleted  " res " items")
             res))))
