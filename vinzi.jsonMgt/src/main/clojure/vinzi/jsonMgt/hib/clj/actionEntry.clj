...
(defrecord actionEntry_clj [id datetime track action user])

(defn create-actionEntry [datetime track action user]
  ;; TO DO:  insert type-checking (java-objects are strictly typed)
  (actionEntry_clj. nil datetime track action user))

(defn clone-actionEntry [cRec]
  ;; TO DO:  insert type-checking cRec should be same type/class as target
  (create-actionEntry (:datetime cRec) (:track cRec) (:action cRec) (:user cRec)))

(defn actionEntry-to-java [cRec]
	(actionEntry. (:id cRec) (:datetime cRec) (:track cRec) (:action cRec) (:user cRec) ))


(add-to-java (actionEntry_clj. 0 1 2 3 4) actionEntry-to-java)

(defn actionEntry-to-clj [jRec]
	(actionEntry_clj. (.getId jRec) (.getDatetime jRec) (.getTrack jRec) (.getAction jRec) (.getUser jRec) ))
