...
;;  moved to vinzi.json.jsonDiffPatch 
                            ...
                            (if2 (not (seq sm))
                                 ...
                                 (let [
                                   ...
                                   (if2 (diff_dm (str (name (ffirst so)) "<->" (name (ffirst sm))) (= 0 keyCompare))
                                        ;; same key, so compare values to detect changes
                                        ...
                                        (if2 (> keyCompare 0)   ;; key so > key sm
          ...
          getVectPat (fn getVectPat [so sm p pl]
                       ...
                       (if2 (not (seq so))
                            ...
                            (if2 (not (seq sm))
                                 (let [nPatches (deletePatch (first so) p pl)
                                       tail     (when (next so)
                                                  (cons (first so) (next (next so))))] ;; reorder first and second (for index of deletion
                                   (recur tail nil nPatches pl))
                                 ;; 
                                 ;; both so and sm are non-empty
                                 (let [o (first so)
                                       m (first sm)
                                       _  (println "getVectPat1: o=" o 
                                                   "\n\tand m=" m
                                                   "\n\tand patches=" p)
                                       oz (nth o 2)
                                       mz (nth m 2)
                                   ...
                                   (println "getVectPat2: oz=" oz
                                            "\n\t mz=" mz
                                            "\n\t returned newPatches=" newPatches)
                                                 ...
                                                 (let [cand  (nth (first no) 2)
                                                   ...
                                                   (if2 (empty? candPatches)
                                                        ...
                                                        (let [delPatches (last (take (inc numDel)
                                                          ...
                                                          (println "getVectPat3: running renum -" numDel " on patches: " newPatches)
                ...
                (if2 (nil? orgNode)   ;; org is nil (and mod is not nil) so insert a value
                     ...
                     (if2 (and (map? orgNode)  ;; Both are compound values (map or vector!!)
                          ...
                          (if2 (= (jsonType org) (jsonType mod)) 
                               ...
                               (let [rec  [(jsonKey org) modNode] ]
                                 (println "getVectorPatches1: insert change-rec using modKey: " rec
                                          "\n\tmodKey: " (jsonKey mod) "  orgKey:" (jsonKey org))
                          ...
                          (if2 (or (map? orgNode)  
                               ...
                               (if2 (= (str orgNode) (str modNode))  
                                    ...
                                    (let [;; NOTE: check whether we can still end up here for a vector!!
                                          ...
                                          rec  [(jsonKey org) modNode]
                                          _    (println "getVectorPatches2: insert change-rec using modKey: " rec
                                                        "\n\tmodKey: " (jsonKey mod) "  orgKey:" (jsonKey org))
    ...
    (if2 (seq patches)
         (let [cPatch (diff_dm "Processing patch " (first patches))
           ...
           (if mod
             ...
             (do
               (patchErr (str "Apply failed on patch " cPatch " (patch ignored)"
                              " for key=" key " path="  pathList " action=" action " and value=" value))
